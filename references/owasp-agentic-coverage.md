...
|-----|------|----------|---------|
| ASI01 | Prompt Injection / Goal Hijacking | PARTIAL — intercepts dangerous command patterns before execution; blocks on critical risk score; **A1:** shell-AST tokenizer now closes G1 (base64-pipe, IFS bypass, eval+dynamic, variable-as-command, network process substitution); does not detect NLP-level injection in prompt text | `claude/hooks/dangerous-command-gate.js`, `runtime/decision-engine.js`, `runtime/shell-ast.js` |
