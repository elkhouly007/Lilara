...
| G5 | 0.2.0 DoD #5 (§10, §17) | Hermes adapter + real-run FP/FN at hard exceptions | **Hermes adapter BUILT 2026-06-17** (clean-room from public docs, MIT, handler-wrap integration). **Real-run FP/FN measured 2026-06-17 (P2.5):** OpenClaw 21/21 cross-harness commands × 6 harnesses matched (0 FP / 0 FN) + 6/6 openclaw adapter fixtures PASS; Hermes 4/4 hermes adapter fixtures PASS + F27 secret-egress + F3 rm-rf BOTH blocked in installed-handler-wrap (LILARA_ENFORCE=1) — FP 0.0% / FN 0.0% on the canonical hard-exception patterns. **Coverage gap (known):** `check-cross-harness-equivalence.sh` does NOT yet include hermes in the 6-harness list (was 6 before hermes landed; harness list update is a separate additive gate-coverage task, NOT part of P2.5). Severity drop: **Med → Low**. | **Low** (was Med) |
