...
- 402/402 fixtures after ADR-036 inviolable tier: F27 `secret-egress-external` (credential/key → external host hard-stop) + F20 floor-codes policy-edit fixture; 22 inviolable floors locked with integrity hash + unreachability proofs; FP 0.0% / FN 0.0%.
- 406/406 fixtures after P2.6 (Hermes doc-honesty clean-up, 2026-06-17): 4 new hermes fixtures (safe-ls, dangerous-rm-rf, enforce-rm-rf, borderline-sudo) matching the opencode/openclaw pattern; hermes block wired in `scripts/run-fixtures.sh`; all 13 local gates green; 7-harness parity enforced. No security-layer change.
- 107/107 scripts after 2026-06-21 housekeeping (branch-hygiene.sh added; dry-run-only by default, --apply to delete merged remote + stale local branches). No security-layer change.
