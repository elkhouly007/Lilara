...
├── output-exfil.js         F19 output-channel exfiltration guard (ADR-010). Inspects outbound channels for taint-class payloads; blocks/escalates per lattice rung.
...
├── notify/                 ADR-015 transports (zero third-party deps): discord.js (node:https), slack.js (node:https), email.js (node:net+node:tls). Creds via env only.
├── post-adapter-factory.js Shared factory consumed by every harness's hooks/post-adapter.js. Calls scanSecrets() + recordExternalRead(); consumes any reported F15 envelopes.
...
10.5  validity-window (outside activeHoursUtc/
      activeDays AND payloadClass!=allow)     no           to block      yes (F11)
10.6  mcp-deny (scopes.mcp[<server>]='block') no         to block      yes (F12)
10.7  skill-deny (scopes.skills[<n>]='block') no         to block      yes (F13)
10.8  budget-exceeded (scopes.budget limit hit) no        to block      yes (F14)
10.9  session-over-duration (scopes.session.maxDurationMin exceeded)
                                              no           to require-review yes (F14b, D47)
...
├── snapshots/                      ADR-013 pre-action state snapshots keyed by receipt id
├── session-budget/                 Per-session destructive-op + external-byte counters (one file per session-id)
├── cross-agent-locks/              F17 per-target locks (atomic create-on-acquire, mtime-bound release)
├── degraded-markers/               ADR-004 degraded-mode entry markers (per process lifetime)
...
- Node builtins: `node:fs`, `node:path`, `node:crypto`, `node:os`, `node:zlib`, `node:https`, `node:net`, `node:tls`
- Local `./` requires within `runtime/`

The `https`/`net`/`tls` builtins are confined to `runtime/notify/{discord,slack,email}.js` — the only outbound surface in the codebase. The decision path itself never opens a socket.

CI enforces this via `scripts/check-zero-deps.sh`. Any PR adding a third-party `require()` in `runtime/` fails CI immediately.

## 8. Post-Decision Pipeline (Stage A–D)

After `decide()` returns, the engine assembles the decision result and runs a fixed post-decision pipeline before returning to the adapter:
...
- **Stress** (`tests/stress/`, `.github/workflows/stress-nightly.yml`): 8 graceful-degradation scenarios run nightly. Each writes per-scenario receipts + journal artifacts to `artifacts/stress/<id>/`.
- **Adversarial** (`tests/adversarial/`, `.github/workflows/adversarial-nightly.yml`): full G+Q pattern library exercised through `decide()` in isolated state dirs. Zero-bypass assertion enforced in CI. `scripts/adversarial-weekly-summary.sh` aggregates the last seven nightly receipts.
