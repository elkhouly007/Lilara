#!/usr/bin/env node
// post-adapter.js — Claw Code PostToolUse adapter for Agent Runtime Guard (best-effort).
//
// Claw Code hook API is not publicly documented. This adapter covers the most likely
// PostToolUse payload shapes. Test against your actual Claw Code hook payload.
//
...
readStdin()
  .then((raw) => {
    ...
    if (!rateLimitCheck("clawcode-post-adapter")) {
      process.stdout.write(raw);
      return;
    }
    ...
    try {
      ...
      if (hit) {
        ...
        try { hookLog("clawcode-post-adapter", "WARN", hit.name); } catch { /* log I/O is non-fatal */ }
      }
