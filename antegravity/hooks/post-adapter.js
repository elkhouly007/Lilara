#!/usr/bin/env node
// post-adapter.js — antegravity PostToolUse adapter for Agent Runtime Guard (best-effort).
//
// antegravity hook API is not publicly documented. This adapter covers the most likely
// PostToolUse payload shapes. Test against your actual antegravity hook payload.
//
...
readStdin()
  .then((raw) => {
    ...
    if (!rateLimitCheck("antegravity-post-adapter")) {
      process.stdout.write(raw);
      return;
    }
    ...
    try {
      ...
      if (hit) {
        ...
        try { hookLog("antegravity-post-adapter", "WARN", hit.name); } catch { /* log I/O is non-fatal */ }
      }
