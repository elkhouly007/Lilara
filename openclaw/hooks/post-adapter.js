#!/usr/bin/env node
// post-adapter.js — OpenClaw PostToolUse adapter for Agent Runtime Guard.
//
// OpenClaw primary shape: { "tool": "shell", "cmd": "...", "output": "...", "cwd": "..." }
...
//   2. Taint record: annotates external-source outputs (fetch, browser, mcp, curl)
...
//
// NOTE: OpenClaw PostToolUse event model is unverified against a live instance.
// This adapter covers the most likely payload shapes. Test against your actual
// OpenClaw hook payload before relying on this in production.
...
readStdin()
  .then((raw) => {
    ...
    if (!rateLimitCheck("openclaw-post-adapter")) {
      process.stdout.write(raw);
      return;
    }
    ...
    try {
      ...
      // OpenClaw shape: output in `output`, `result`, or `content`
      const toolName   = String(input.tool || input.tool_name || "");
      ...
      if (hit) {
        ...
        try { hookLog("openclaw-post-adapter", "WARN", hit.name); } catch { /* log I/O is non-fatal */ }
      }
