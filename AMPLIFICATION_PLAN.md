...
G7. Contract scope expansion: per-MCP-tool, per-skill, time-windowed, branch-aware, session-bounded scopes exist partially in v2 but are not consumed end-to-end by `decide()` and the CLI. [validity wired] [contextTrust wired] [perToolAllow wired]
