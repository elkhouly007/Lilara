...
- **chore(p3-1): SCOPE §19 #6 laundering regression gate** — NEW `scripts/check-inviolable-tier-unreachability.sh` (~190 lines). Elevates the two unreachability tests under `tests/decision-lattice/inviolable-{selfmod,contract}-unreachability.test.js` to a standing CI gate, AND adds the (b) monotonic-rebaseline check: any change to `runtime/decision-lattice.js` or `runtime/floor-codes.js` in a commit range must be accompanied by an `artifacts/lattice-baseline.sha256` rebaseline AND a `[LATTICE-BASELINE-REBASELINE]` marker in `CHANGELOG.md` (so the rebaseline is reviewable, not silent). The script is exit-code-based (0/1/2); axis (a) runs the unreachability structural check in-process via `node -e` (22 inviolable floors × 11 demotion sources, plus `assertOrdered` on the live `LATTICE`); axis (b) uses `git diff --name-only` over the merge-base range. Wired into `.github/workflows/check.yml` as a new CI step. **`scripts/check-counts.sh` `EXPECTED_SCRIPTS` bumped 104 → 105** (script lives at `scripts/` maxdepth 1, so the count moved). No `runtime/`, no adapter, no floor, no lattice, no replay-corpus change. No `[LOCKED]/[ADVISORY]/[OPEN]/[CC-PROPOSED]` tag change. No `/root/lilara-handover/` modification. Per `references/SCOPE.md` §19 #6 + §12 + `references/PLAN.md` Phase 3.1. Tooling-only change.
