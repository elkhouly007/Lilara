...
- **A2 — Provenance/taint tracking for indirect prompt-injection defense** (`runtime/taint.js`, `runtime/provenance-correlator.js`): `recordExternalRead(content, source)` annotates tool results from external sources (browser, MCP, web-fetch, curl/wget) to a 5-minute provenance window at `~/.horus/provenance-window.json` (max 20 entries, mode 0600). `correlateCommand(command)` checks for overlap using exact substring match or token-level match (tokens ≥ 6 chars, non-flag). When overlap is found, `decision-engine.js` raises the action to `require-review` via the **F10 taint floor** (rung 8.5 — after protected-branch floor, before session-risk floor), setting `decisionSource = "taint-floor"`. Journal records carry `taintSource` and `taintReason` fields on tainted decisions. Zero external dependencies; taint errors never block a decision (try/catch, best-effort). `runtime/session-context.js` gains `recordExternalRead`, `getProvenanceWindow`, and `provenanceWindowPath` exports. Acceptance: 4 inline tests in `run-fixtures.sh` (tainted→require-review, unrelated→allow, clean-window→not-taint-floor, journal-fields present). A2 acceptance read narrowly per 2026-05-07 agreement: Claude-harness fixture only. A3 will port the PostToolUse hook to the other 5 harnesses. Closes G2 (indirect prompt injection) from ENHANCEMENT_PLAN.md.
