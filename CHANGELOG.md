...
### [LATTICE-BASELINE-REBASELINE] Floor — F30 runtime tamper floor (ADR-050)

- **feat(tamper-floor): F30 `tamper-floor` — inviolable block over installed guard footprint** — `[LATTICE-BASELINE-REBASELINE]` — NEW runtime tamper floor (F30, ADR-050, rung 18.75, tier inviolable). Path-set anchored on state-paths.js resolved dirs (`stateDir()`, `hookStateDir()`, `instinctDir()`) — prefix-match with `path.sep` boundary, no hardcoded file list. `demotableBy: []`; legitimate updates route through installer/upgrade path as operator-on-host, OUTSIDE agent session (SCOPE §21 host-trust boundary). Pure predicate (no I/O); `evaluateFloor(input)` returns `{ fired: true, action: "block", reason }` when input.targetPath resolves under a protected dir, else `{ fired: false }`. **ADR-050 compliance (load-bearing):** the floor decides on the IR's resolved `targetPath` ONLY — it does NOT parse `input.command` for path tokens (would bypass the IR contract, defeat the "pure input" guarantee, and false-positive on legitimate commands whose argv merely mentions a protected path). Engine wiring via `floor-tamper.js` module (F19/output-exfil.js encapsulation pattern); F30 fires in the rung-18.75 block band after F28 and before F16. `INVIOLABLE_FLOOR_IDS` auto-derived (21→22 inviolable floors). NEW `runtime/floor-tamper.js` (~78 LOC); NEW `tests/runtime/floor-tamper.test.js` (14 cases: block/allow/override/subdir/unrelated-prefix + 7 ADR-050 compliance cases proving no command-token parsing). NEW `scripts/check-f30-tamper-floor.sh` (~170 LOC, 6 sub-checks: lattice + floor-codes + evaluateFloor behavior + engine wiring + INVIOLABLE_FLOOR_IDS count + unit tests); wired into `.github/workflows/check.yml` after the F29 delete-coord gate. Lattice baseline rebaselined (both `lattice` and `floor-codes` hashes). §19 #6 precondition satisfied on master.
- **fix(tests): PR-F anti-mask refactor — keep `cwd`/HOME separate from `LILARA_STATE_DIR` in 4 tests** — Three test files (`tests/runtime/degraded-mode.test.js`, `tests/runtime/journal-chain.test.js`, `tests/runtime/consent-f6-prompt-wiring.test.js`) set `process.env.HOME = process.env.LILARA_STATE_DIR = <tmp>` at module scope; `tests/runtime/taint-window-injection.test.js` then passed `cwd: <LILARA_STATE_DIR>` to `runPreToolGate`, which makes `pretool-gate.js:209` set `targetPath = cwd` → IR carries `LILARA_STATE_DIR` as targetPath → **F30 would correctly block first and mask the floor under test (F10 taint, F28 fail-closed)**. Refactored: each test now uses TWO `mkdtempSync` dirs — `stateDir` for journal/chain/decision writes, `repoDir` (or `tmpHome`) for the test repo's cwd (or HOME override). Anti-mask proof added per case: each refactored case asserts the INTENDED floor's `floorFired`/action/state, not F30. `scripts/check-runtime-core.sh` `mktemp -d` pattern also updated to keep HOME and LILARA_STATE_DIR in separate tmp dirs (sample-repo / shape-repo / f15-envelope-repo test repos no longer land inside F30's protected footprint). No production code change. No `[LOCKED]/[ADVISORY]/[OPEN]/[CC-PROPOSED]` tag change.

### CI — De-mask status-summary.sh exit code + Path B explicit capture contract + superiority-evidence layer-count regex

- **fix(ci): de-mask status-summary.sh + env-var Path B capture + superiority-evidence regeneration** — `scripts/status-summary.sh` now propagates `[Verification]` sub-check failures to its process exit code instead of the old `… && printf ok || printf FAILED` pattern that always exited 0 (Pitfall 53, surfaced by **#195**). NEW `run_check NAME CMD…` wrapper + `declare -i fail` accumulator + terminal `exit "$fail"`; `set -o pipefail` scoped to THIS wrapper only (not the sub-scripts). The `audit-examples` non-standard manual-review guard is preserved as an inline `if` that still trips the fail flag, and the `ARG_SKIP_STATUS_ARTIFACT_CHECK` skip path is preserved as a non-failure ("status-artifact: skipped"). `scripts/generate-status-artifact.sh` gains an explicit **Path B** capture contract via `ARG_STATUS_SUMMARY_CAPTURE` (an absolute path OUTSIDE `$out_dir`, mirroring Path A's `ARG_STATUS_SUMMARY_FILE`), copied into `$out_dir`; the hardcoded `/tmp/status-summary-capture.txt` is retained as a backwards-compat fallback, and `.github/workflows/check.yml`'s "Check status artifact" step now passes the capture path explicitly via the env var. `scripts/generate-superiority-evidence.sh`'s verification-layer count regex is updated from `^\s*\./scripts/` to `(^|\s)\./scripts/`: the de-mask reshaped each invocation from a line-leading `./scripts/foo.sh …` into a `run_check "name" ./scripts/foo.sh` wrapper, so `./scripts/` is no longer line-leading and the old anchor matched 0 post-de-mask. The de-mask is a **pure reshape** (no verification layer added or removed), so the faithful layer count is unchanged at **24** (23 `run_check` invocations + 1 inline `audit-examples` guard) and `references/superiority-evidence.md` is byte-identical — no `.md` content change. No `runtime/`, no adapter, no floor, no lattice, no replay-corpus, no `decide()` change. No `[LOCKED]/[ADVISORY]/[OPEN]/[CC-PROPOSED]` tag change. No `[LATTICE-BASELINE-REBASELINE]` marker. CI-tooling only.
  - **fix(ci): skip self-recursive status-artifact inner check (Option B)** — `.github/workflows/check.yml`'s "Status summary" step now prefixes the `status-summary.sh` invocation with `ARG_SKIP_STATUS_ARTIFACT_CHECK=1` to break a self-recursive race. The inner status-artifact check is self-referential — it runs before status-summary prints the `[Parity Snapshot]` section it validates, so it can never pass; under the de-mask it propagated to exit 1 and failed all 6 CI jobs across all 3 OSes. The authoritative gate remains the dedicated "Check status artifact" step, which runs `check-status-artifact.sh` on the COMPLETE capture. The skip suppresses ONLY this structurally-impossible inner check; the 23 other `run_check` sub-checks remain honestly gated and still propagate failure. Workflow + CHANGELOG only; `status-summary.sh` (the skip branch) unchanged this commit.
...
### [SECURITY-LAYER][INERT] F27-specific transport integration test (no-self-approve + no-TTY invariants locked for F27 path)

- **test(f27-transport): lock no-self-approve + no-TTY invariants for the F27 secret-egress path** — NEW `tests/runtime/floor-f27-consent-transport.test.js` (5 cases) proving that when an F27-shaped decision (`floorFired:"secret-egress-external"`, `decisionSource:"secret-egress-consent-required"`, `f27Consent:{host,credentialClass}`) reaches `transport.requestConsent`, the two structural transport invariants fire on the F27 path specifically: (2.1) no TTY → fail-closed `deny`; (2.2) `__LILARA_CONSENT_TEST_AUTO=1` with `NODE_ENV=production` stays `deny` (auto-approve sentinel inert outside `NODE_ENV=test` — no env-var self-approve); (2.3) the operator-facing prompt names the destination via the byte-exact canonical F.6 line (`A secret/API key is about to be sent to evil.example.com - approve?`, sourced from REAL `f27Consent.host`); (2.4) the F27 path never reads `process.stdin` (every read surface boobytrapped to throw — no-self-approve); (2.5) F27-contextual re-assertion that `transport.js` carries zero `process.stdin` references and reads `/dev/tty` only. **TEST-ONLY** — no production code changed; `runtime/consent/transport.js` and the canonical F.6 string are untouched. Closes the gap that the existing generic transport tests (F18 network-egress) and `floor-f27-inert.test.js` (engine no-TTY branch) leave open. No lattice touch (no `[LATTICE-BASELINE-REBASELINE]` marker).

### Feat — F.6 secret/API-key egress consent-prompt wiring

- **feat(f6-wiring): render the F.6 destination line for F27 + F28 consent prompts** — Wires the contract's F.6 stop-and-ask line (`"A secret/API key is about to be sent to <destination> - approve?"`) into the consent transport. `runtime/consent/transport.js`: `buildConsentPrompt` now derives `secretEgressHost` (F27 → `decision.f27Consent.host`; F28 → `decision.taintEgress.host`, falling back to `decision.networkEgress.hostname`), `secretEgressCredClass`, and a `noDestination` fail-closed flag, all from REAL decision fields only (never agent self-description); `buildPromptText` prepends the F.6 line **full-width / un-boxed above the consent box** so the destination is never truncated. Applies to **both** secret-egress floors (F27 `secret-egress-external` and F28 `taint-egress-consent`) — additive only: F28's existing `Tainted:`/`Class:` framing is unchanged. **Fail-closed:** when a secret-egress floor fires but no destination can be named, `pretool-gate.js` downgrades the receipt from `consent-required` to a hard block (`BLOCKED — secret/API-key egress with no nameable destination`) instead of presenting a blind consent prompt. **Canonical string is byte-identical** across the code, the new test, `CONTRACT.md`, and `RED-LINES.md` (ASCII: capital A, tight `secret/API key` slash, ASCII HYPHEN-MINUS before `approve?`); `CONTRACT.md` and `RED-LINES.md` were normalized off the prior em-dash/spaced-slash form. NEW `tests/runtime/consent-f6-prompt-wiring.test.js` (7 cases, incl. full-width-no-truncation, byte-exact canonical form, fail-closed null-host, and a negative self-description-injection test). Prompt-RENDERING change ONLY — no decision-engine firing semantics, no floor changes, no lattice touch (no `[LATTICE-BASELINE-REBASELINE]` marker). No-self-approve / no-TTY transport invariants unchanged.
  - **f6-wiring: 'unknown' host sentinel treated as non-nameable (Misk review follow-up)** — F28 can emit the literal sentinel `host: "unknown"` (via `runtime/floor-taint-egress.js:181` from `runtime/provenance-graph.js:263` `classifySink` when a network-send shape has no parsed host). Treats "unknown" as ABSENCE of a destination, NOT a nameable one. NEW helper `isNameableHost(h)` in `runtime/consent/transport.js` (returns `false` for `null`/`""`/`"unknown"`); the F.6 destination derivation now uses `[f27Consent.host, taintEgress.host, networkEgress.hostname].find(isNameableHost) || null` so any "unknown" sentinel in the chain downgrades to `noDestination:true`, which makes `buildPromptText` skip the F.6 line and `pretool-gate.js` block before `requestConsent`. **Scope strictly limited** to the F27/F28 secret-egress F.6 derivation — non-secret-egress floors (e.g. F18 network-egress) still see `networkEgress.hostname === "unknown"` rendered in their ordinary `Host:` line unchanged. Two-layer regression coverage: (1) `consent-f6-prompt-wiring.test.js` cases "F28 with host 'unknown'..." and NEGATIVE "non-secret-egress floor with networkEgress.hostname 'unknown'..." (transport-level); (2) "F28 with host 'unknown' through the real gate..." drives `runPreToolGate` end-to-end and asserts `exitCode === 2, logAction === "BLOCK"` plus the fail-closed stderr (gate-level). Pre-existing `gateIr` scoping defect in `pretool-gate.js` also fixed (minimal hoist to function scope) so the gate-level guard is reachable — the defect predates F.6 (added by an earlier F29 ADR-038 line) but was within the F.6 fail-closed blast radius. **Windows CI latency regression investigation:** the two Windows notify-transport.test.js fails (492ms, 240ms over 200ms budget) on PR #189 were traced to Windows runner noise — `transport.js` and `pretool-gate.js` are NOT in `decide()`'s 48-module import graph (closure byte-identical to master); F.6 cannot affect `decide()` latency by any code path. NO code fix for the Windows issue; `tests/runtime/notify-transport.test.js` budget UNTOUCHED (the test correctly protects the fire-and-forget property).

### Floor — F27 reclassification (PR-C)

- **feat: reclassify F27 into demotable tier** — `[LATTICE-BASELINE-REBASELINE]` — PR-C F27 reclassification: tier inviolable->demotable, demotableBy []->[consent:interactive], action block->escalate. Lattice projection hash legitimately changes; floor-codes.js unchanged.
  - **F27 stale-site sweep (held-branch CI-fix, owner-approved 2026-06-22):** F27 reclassification invalidated 7 sibling sites that mirrored F27's old classification or asserted F27 enforcementAction=="block" (which legitimately changes under PR-C). Sanad APPROVED at engine layer (F27 block runs before the line-1412 consent-suppression path and reads only LILARA_F27_CONSENT — a forged consentGrant cannot reach F27's decision). Sites: (1 A) `INVIOLABLE_FLOOR_IDS` membership in `inviolable-contract-unreachability.test.js`; (6 B, preserved narrowly) forged-consent protections in `inviolable-contract-unreachability.test.js` + `inviolable-selfmod-unreachability.test.js` (action==="block" strictEqual UNCHANGED, enforcementAction===`"block"` -> `!=="allow"`); F27 anchor swap to F3 (still inviolable post-PR-C, no coverage lost) in `_INVIOLABLE_AT_LOAD` depletion-immunity + lattice-hash tamper + `assertOrdered` tests; `scripts/check-taint-egress.sh` regression guard inverted (F27 must be demotable by consent:interactive), F28 shape assertions + behavioral checks 5-9 UNTOUCHED; `tests/fixtures/replay-corpus/build-secret-egress-adversarial.js` secondary guard loosened (PRIMARY guard at line 209 unchanged — action==="block" still hard-required). 14/14 adversarial corpus cases pass; forged-consent case records action:"block" despite maximally-permissive injected grant. **This CI-fix force-push is NOT the PR-C merge review** — full Misk + Sanad PR-C merge review governs the irreversible merge, after the three preconditions land.

### Chore — Branch hygiene (housekeeping, 2026-06-21)

- **chore(branch-hygiene): add `scripts/branch-hygiene.sh` to enumerate and (with `--apply`) delete merged remote + stale local branches** — 12 remote branches fully merged into origin/master (no open PR, not base of any PR) + 3 local stale branches marked for deletion; protected patterns (`feat/f27-*`, `feat/adr-054-*`, `phase-*`) NEVER deleted. **Dry-run by default** (lists candidates, exits 2); `--apply` required to actually delete (with `--yes` to skip prompt). Skips any branch whose tip is not an ancestor of master or that has no upstream tracking AND is not merged (e.g. local `chore/p3-1-inviolable-tier-regression-gate` correctly stays because the local tip is the rebaseline-marker rule commit, not merged to master). Skips any remote branch with an OPEN PR (head or base reference). **`scripts/check-counts.sh` `EXPECTED_SCRIPTS` bumped 106 → 107** (script lives at `scripts/` maxdepth 1, so the count moved). **`README.md` "Scripts (scripts/) — 106 files" → "107 files"**. **`references/full-power-status.md` new entry: "107/107 scripts after 2026-06-21 housekeeping"**. No `runtime/`, no adapter, no floor, no lattice, no replay-corpus, no `decide()` change. No `[LOCKED]/[ADVISORY]/[OPEN]/[CC-PROPOSED]` tag change. No `/root/lilara-handover/` modification (only the unrelated `SOUL.md.pre-revert-bak` backup file was removed — see package-state audit at `/root/lilara-reviews/2026-06-21-soul-md-pre-revert-bak-removal-record.md`).

### Chore — Hermes cross-harness coverage (P2.5 follow-up)

- **chore(hermes-cross-harness): add hermes to check-cross-harness-equivalence** — Adds `hermes` to the harness list in `scripts/check-cross-harness-equivalence.sh`, closing the P2.5 cross-harness coverage gap ("check-cross-harness-equivalence.sh does NOT yet include hermes in the 6-harness list; was 6 before hermes landed; harness list update is a separate additive gate-coverage task, NOT part of P2.5"). 21 representative commands × 7 harnesses (claude, openclaw, opencode, codex, clawcode, antegravity, hermes) matched — exit 0. Hermes uses handler-wrap integration (see `hermes/WIRING_PLAN.md`) but exercises the same `pretool-gate` spine, so equivalence holds. No `runtime/`, no adapter, no floor, no decide, no lattice, no replay-corpus change. No `[LOCKED]/[ADVISORY]/[OPEN]/[CC-PROPOSED]` tag change. No `/root/lilara-handover/` modification. Per P2.5 follow-up; closes a known coverage gap.

### Chore — §19 #14 replay posture pinning + posture-matrix gate (P3.3)

- **chore(p3-3): SCOPE §19 #14 replay posture hardening** — Pins the three posture flags that `decide()` reads from ambient env (`LILARA_TAINT_EGRESS`, `LILARA_DELETE_COORD`, `LILARA_KILL_CHAIN_ENFORCE`) explicitly to `"0"` in `scripts/replay-decisions.js` so the replay harness is posture-deterministic regardless of the ambient CI env. Adds `LILARA_REPLAY_RESPECT_POSTURE=1` opt-out for callers (the posture-matrix gate) that need to exercise the matrix. NEW `scripts/check-replay-posture-matrix.sh` (~151 lines) replays every shipped corpus file under all 8 combinations of the three flags. **The gate is two-faced**: the canonical baseline (all-off, TE=0 DC=0 KC=0) MUST be zero-drift and hard-fails (exit 1) on any drift; the other 7 combinations report their posture surface (drift is the F29 floor engaging, not a regression) and exit 0. **Result: 48 runs (8 combinations × 6 corpora); canonical baseline is zero-drift; the 7 non-canonical combinations report 5 entries drifted (DELETE_COORD=1 engaging the F29 floor — legitimate posture surface, not a regression).** Per-posture canonicalization (zero-drift required everywhere) is documented as `NEEDS-APPROVAL` future work in the script header, not bundled. Wired into `.github/workflows/check.yml` as a new CI step after "Decision replay". `scripts/replay-decisions.js` source change is the only one; no `runtime/`, no adapter, no floor, no lattice, no replay-corpus (entries) change. No `[LOCKED]/[ADVISORY]/[OPEN]/[CC-PROPOSED]` tag change. No `/root/lilara-handover/` modification. Per `references/SCOPE.md` §19 #14 + `references/PLAN.md` Phase 3.3.

### Chore — §19 #6 inviolable-tier regression gate (P3.1)

- **chore(p3-1): SCOPE §19 #6 laundering regression gate** — NEW `scripts/check-inviolable-tier-unreachability.sh` (~250 lines). Elevates the two unreachability tests under `tests/decision-lattice/inviolable-{selfmod,contract}-unreachability.test.js` to a standing CI gate, AND adds the (b) monotonic-rebaseline check: any change to `runtime/decision-lattice.js` or `runtime/floor-codes.js` in a commit range must be accompanied by an `artifacts/lattice-baseline.sha256` rebaseline AND a `[LATTICE-BASELINE-REBASELINE]` marker in `CHANGELOG.md` (so the rebaseline is reviewable, not silent). The per-commit rule (v2): every commit that touches a tier file or the baseline must have the marker in its OWN `CHANGELOG.md` diff — this prevents a stacked silent rebaseline from inheriting a parent commit's marker. The script is exit-code-based (0/1/2); axis (a) runs the unreachability structural check in-process via `node -e` (22 inviolable floors × 11 demotion sources, plus `assertOrdered` on the live `LATTICE`); axis (b) uses `git rev-list` over the merge-base range to enumerate tier-touching commits. Wired into `.github/workflows/check.yml` as a new CI step. **`scripts/check-counts.sh` `EXPECTED_SCRIPTS` bumped 104 → 105** (script lives at `scripts/` maxdepth 1, so the count moved). **`README.md` "Scripts (scripts/) — 104 files" → "105 files"** (count-drift fix caught by `check-status-docs`). No `runtime/`, no adapter, no floor, no lattice, no replay-corpus change. No `[LOCKED]/[ADVISORY]/[OPEN]/[CC-PROPOSED]` tag change. No `/root/lilara-handover/` modification. Per `references/SCOPE.md` §19 #6 + §12 + `references/PLAN.md` Phase 3.1. Tooling-only change.

### Chore — Reviewer GATE 5 content-aware enforcement (review tooling)

- **chore(review-criteria): content-aware GATE 5 wrapper** — NEW `scripts/review-criteria/gate5-content-aware.sh` (~110 lines). The earlier GATE 5 criterion (literal `in` substring match against raw markdown) was brittle: a row like `Severity drop: **Med → Low**` failed the contiguous-token check because the `**` markers split the text. The wrapper strips markdown formatting (`**`, `*`, `` ` ``, `→`, `←`, `⇒`, extra whitespace) BEFORE the token match, so semantic content is what gets checked — not presentation. Verifies 6 required tokens, 4 preserved tokens, and 4 forbidden tokens (scope-tag markers). **Reviewer prompts now CALL the wrapper instead of inlining the regex by hand.** The wrapper is the single source of truth — the criterion cannot drift between dispatches. Tooling-only change. No `runtime/`, no adapter, no floor, no lattice, no replay-corpus change. No `[LOCKED]/[ADVISORY]/[OPEN]/[CC-PROPOSED]` tag change. No `/root/lilara-handover/` modification. No profile modification. `scripts/check-counts.sh EXPECTED_SCRIPTS` unchanged (script lives in `scripts/review-criteria/`, a subdirectory — `find -maxdepth 1` does not count subdir files). All 17 local gates green (15 baseline + check-status-docs + check-eval-budgets).
...
- **feat(adr-031): input materialization at `decide()` entry — closes trust-boundary A1-A9** — New module `runtime/input-materializer.js` + single `input = _materializeInput(input)` seam after the kill-switch early-return in `decide()`. Produces a plain, getter-free, null-safe copy before any floor, discover(), Object.entries(), or receipt builder reads the input. Closes 9 load-bearing crash surfaces (A1–A9) in one seam without touching floor internals. Before: `decide(null)` / throwing getter → uncaught throw → gate exit 0 (allow). After: adversarial inputs return `escalate` (human-gate decision, never allow). Gates: 0.0%/0.0%, 105/105 replay entries byte-identical. PR #118.

- **feat(adr-032, envelope): `envelope.js` state-dir validation + `loadBaseline` fail-safe** — Fixes the sole fail-UNSAFE state-dir finding. `loadBaseline()` previously re-baselined silently to the current (possibly poisoned) env on missing/corrupt baseline, suppressing the F15 env-divergence floor. Fix: `ensureBaseDirSafe(stateDir())` guards the entire read+write path; unsafe dir → `null` → `diffEnv(null, current)` → all env vars appear as "added" → F15 fires. Also adds state-dir guards to `loadDeclaredEnvelope`, `rememberPending`, `loadPending`. Isolated PR because it changes a floor's fail direction. PR #119.

- **feat(adr-032, sweep): state-dir validation — full consumer sweep (12 remaining modules)** — Extends `ensureStateDirSafe`/`ensureBaseDirSafe` to all 12 remaining unvalidated state-dir consumers found by the comprehensive audit (`journal-chain.js`, `contract.js`, `snapshot.js`, `state-bundle.js`, `receipt-export.js`, `telemetry.js`, `session-budget.js`, `session-memory.js`, `session-resume.js`, `spend-estimator.js`, `workflow-enforcer.js`, `memory-search.js`). The audit found 18 total state-dir consumers; 4 were validated by ADR-028; envelope.js in PR #119; these 12 complete the sweep. Consistent pattern across all files. Gates: 0.0%/0.0%, 105/105 replay entries byte-identical. PR #120.

- **feat(adr-033): mcp-pin — unify `stateDir()` fallback, retire `os.tmpdir()` path** — `mcp-pin.js` was the only state-dir consumer using `process.env.LILARA_STATE_DIR || os.tmpdir()` instead of `stateDir()` (→ `~/.lilara`). On world-writable `/tmp` with the var unset, `ensureStateDirSafe` rejected it → drift detection silently disabled. Also: pins were invisible to state-bundle backup. Fix: replace with `_stateDir()` from state-paths; switch `ensureStateDirSafe` to `ensureBaseDirSafe` in `checkArgShapeDrift` so first-run auto-creates `~/.lilara` at 0o700. No compat shim (Khouly decision). PR #121.

- **feat(adr-021, depth-cap): canonical-json bounded recursion (cap=64, 12.8× corpus max)** — `runtime/canonical-json.js` shared serializer (used by irHash, journal-chain, envelope hash, receipt-export, contract hash, snapshot manifests, notify payloads) previously had unbounded recursion. A deeply-nested IR could stack-overflow the Node process, killing it and leaving the gate unprotected. Cap = 64 = 12.8× measured corpus max depth (5, over 225 corpus entries). `depth > 64` → `RangeError("canonicalJson: depth limit exceeded")` — controlled, catchable, already inside try/catch at all call sites. Probe: depth=64 succeeds, depth=65 throws, 105/105 corpus entries byte-identical. PR #122.

- **docs(adr-034): Proposed ADR — MCP inbound response inspection** — Documents the trust-boundary audit finding that Lilara's MCP inspection surface is outbound-only (PreToolUse). Classic inbound threats (tool-list poisoning, malicious tool descriptions, malicious result payloads) are currently outside the inspection surface by design. Three options analyzed: per-first-call tool-list inspection, PostToolUse blocking gate, proxy mode. Decision pending. `references/adr-034-mcp-inbound-response-inspection.md`.
...
- **docs(adr-022): Proposed ADR — fail-closed F25/F26 floor recovery** — Audit finding (HIGH): `_evalMcpArgFloor` and `_evalMcpRegistrationFloor` end with `catch { return { fire: false } }` (fail-open). An attacker with a getter-throwing `tool_input` object causes the catch to fire, allowing the call. Fix: change outer catches to `return { unscannable: true }` (fail-closed via `require-review`). Awaiting approval. `references/adr-022-fail-closed-floor-recovery.md`.

- **docs(adr-023): Proposed ADR — unified command classification gateway** — Audit finding (HIGH): 4 direct call sites to raw-only `classifyCommand` (`action-ir.js:490`, `contract.js:545`, `decision-engine.js:1039`, `degraded-mode.js:108`) bypass the Unicode dual-path (`_classifyCommandDual`). IR receipts misrepresent command class; contract scope resolution may grant permissions for Unicode-obfuscated destructive commands. Fix: export `classifyCommandDual` from `decision-key.js`; update all 4 sites. Awaiting approval. `references/adr-023-unified-classification-gateway.md`.

- **docs(adr-024): Proposed ADR — state-dir permission validation** — Audit finding (HIGH): `runtime/mcp-pin.js` reads/writes `LILARA_STATE_DIR/mcp-pins/pins.json` without validating directory permissions. Attacker with write access to `LILARA_STATE_DIR` can pre-pin hashes or corrupt the store, suppressing rug-pull drift detection — especially significant after ADR-018 Option 1 makes the pin enforcement-bearing. Fix: validate state-dir ownership/permissions at startup (warn + disable stateful pin, not abort); atomic pin update via write-temp+rename. Awaiting approval. `references/adr-024-state-dir-permission-validation.md`.
...
- **docs(command-normalize): correct stale NFKC references — implementation uses NFKD** — `tests/runtime/command-normalize.test.js` header (line 11), section header (line 177), and one test name (line 195) referenced "NFKC". The implementation (`runtime/command-normalize.js`) calls `s.normalize("NFKD")` followed by an explicit combining-mark strip (`STRIP_RE`). NFKD is the correct form here: it decomposes precomposed characters (e.g. `ṙ` → `r` + U+0307) so that the subsequent combining-mark strip can remove the diacritic. The original "NFKC" would have re-composed rather than stripped. Zero behavior change — comment/header and one test-name edit only.

- **docs(full-power-status): correct stale fixture counts (386→395)** — `references/full-power-status.md` lines 17, 28, and 149 asserted "386/386" as the current fixture count. The live count is 395 (per `find tests/fixtures -name '*.input' | wc -l`). `check-fixture-count.sh` only requires the live count to appear *somewhere* in the doc; lines 17/28/149 were historical current-state claims that went stale as fixtures were added in PRs #72-#90 without touching those specific lines. Updated all three to 395/395. Also corrected line 149's script count from 94 → 95 (check-project-scope.sh was added in the L6 sprint).

### Security

- **feat(adr-018): trusted-server dual-use + rug-pull drift escalation (F25 rug-pull seam)** — A trusted MCP server (`scopes.mcp[server].policy="allow"`) issuing a GATED_REVIEW dual-use command class (`destructive-db` / `auto-download` / `global-pkg-install`) was `allow` regardless of arg-shape drift. An attacker who gains write access to the server binary could pivot the tool's behavior (new arg-key shape) and inject `DROP DATABASE prod` undetected. Fix (ADR-018 Option 1): hoist `checkArgShapeDrift` above F25 (was observe-only advisory after F26); thread the boolean into `_evalMcpArgFloor`. When trusted + GATED_REVIEW dual-use class + drift co-occur, return `{ review:true, reason:"trusted-server-dualuse-after-drift:command-class=<cls>" }` → `require-review` (one human gate, self-healing — pin re-pins on detection). Drift alone never escalates; steady-state DB MCP (stable shape, routine DROP) stays allowed. Stateful pin invoked exactly once per decide() call (threading constraint preserved). New fixture `15-f25-trusted-dualuse-nodrift-allow` (trusted + DROP + no-drift → allow) + inline multi-call fixtures B (drift+dual-use → require-review) and C (drift-only → allow) in `check-mcp-security.sh`. 396 fixtures total (was 395). Eval 0.0%/0.0%; replay corpus byte-identical (first-sight entries unaffected — drift=false on first call).

- **feat(adr-020/confusable): confusable-normalized bypass-pattern hardening (Track B)** — `isBase64PipeExec` and `isNetworkProcessSub` ran on raw strings only; `_classifyCommandDual` already normalizes for `classifyCommand`. An attacker writing `bаse64 -d | sh` (Cyrillic U+0430 'а') or `b‍ase64 -d | sh` (ZWJ U+200D splice) evaded both predicates. Fix: import `normalizeCommand` into `runtime/shell-bypass-detector.js`, add module-level `dual(re, raw, norm)` helper. Both exported predicates compute `norm = normalizeCommand(s)` and route each regex arm through `dual`. `detectBypassPatterns` also computes `norm` and routes `hasBase64Decode` through `dual` (for `hasEvalDynamic`). Pure-ASCII fast-path preserved (`norm === raw` → second arms skip → byte-identical). Fix propagates to all call sites automatically: F25, F26, `risk-score.js`. 4 new fixtures (16-cyrillic-base64-block, 17-zwj-base64-block, 18-cyrillic-proc-sub-block, 19-variable-template-allow). 4 new corpus entries (dangerous-35/36/37, safe-56). 400 fixtures (was 396). 120 corpus entries (was 116). FP 0.0% / FN 0.0%. ADR-020 follow-up closed.

- **feat(adr-020/narrow): F25/F26 MCP bypass-pattern parity — base64-pipe-exec and network-process-sub** — The MCP danger floors (F25 `mcp-arg-danger`, F26 `mcp-registration-write`) classified arg/config strings only through `classifyCommand` (9-regex set) and never invoked `runtime/shell-bypass-detector.js`. An MCP tool whose arg contained `echo … | base64 -d | sh` or `bash <(curl …)` resolved to `route`/risk-4 instead of `block`/risk-10 (verified: `decide()` on MCP `{cmd:"echo Zvm | base64 -d | sh"}` → `route` risk=4, vs Bash control → `block` risk=10). Fix (ADR-020 Option 2 — narrow scope): extract the two unambiguous predicate regexes from `detectBypassPatterns` into named exported functions `isBase64PipeExec` / `isNetworkProcessSub` in `runtime/shell-bypass-detector.js` (hoisting existing literals to module consts; `detectBypassPatterns` refactored to call them — byte-identical Bash-path behavior, no replay drift). F25 and F26 import and invoke only these two predicates before `_classifyCommandDual` in all scan loops (structured-JSON and raw-value fallback in F26; arg-string loop in F25). The graduated outcome mirrors the existing HARD_BLOCK path: untrusted server → `block` (`mcp-arg-danger-denied` / `mcp-registration-write-denied`); trusted (opted-out) server → `require-review`. Wider patterns (variable-as-command, bare-`$()` unresolvable) stay out per the ADR FP analysis — they appear legitimately in SQL/template MCP data. Five new fixtures: `10-f25-base64-pipe-exec` (block F25), `11-f25-network-process-sub` (block F25), `12-f26-base64-pipe-launch` (block F26), `13-f25-safe-sqltemplate-cmdsub` (anti-FP allow), `14-f25-safe-base64-data` (anti-FP allow). 395 fixtures total (was 390). Eval corpus 114→116 (`dangerous-34` MCP base64-pipe block, `safe-55` base64-data allow). FP 0.0% / FN 0.0%.



- **feat(L6): project-scope the learned-policy store — close the cross-project trust leak** — `runtime/policy-store.js` keyed `learnedAllows` (and `approvalCounts` / `suggestions` / `autoAllowOnce`) by the global `fineKey` (`tool|commandClass|pathBucket|branchBucket|payloadClass`). The path bucket is project-*relative*, so two projects that share an internal layout produced byte-identical keys: an approval for `rm -rf build/` — or **any** command that collapses to `default-target` because it has no project-distinguishing path (e.g. `sudo systemctl restart …`, `git push --force`, `npm i -g …`) — recorded in a throwaway project silently applied to a matching action in a production project. (The earlier relative-path-bucket change only *narrowed* this; it did not close it — verified empirically: `bash|sudo|default-target|protected-branch|A` is identical across all projects.) Now every learned-policy lookup is prefixed with a per-project scope tag derived by the new `runtime/project-scope.js`, layered most-stable-first: git `remote.origin.url` (survives directory renames + re-clones) → git work-tree toplevel → absolute project-root path hash → `null`. When no stable project identity is derivable the lookup returns `null` and **never matches** — strictly stricter than before, never a fall-through to the old global key. The unscoped, human-readable receipt `policyKey` is intentionally unchanged, so the byte-identical replay corpus and audit receipts are unaffected.

  **Operator impact (breaking for stored learned-policy state):** learned-allow grants, approval counts, and pending suggestions recorded by an earlier version are **orphaned** — they live under the old global key format, which a project-scoped lookup can never match, so they will no longer be honored and must be re-approved *within each project*. Nothing is deleted: legacy entries remain inert in `learned-policy.json`. This is the cheapest moment to migrate the key shape (pre-launch, small approval history); the migration cost grows with every accumulated approval. The chosen migration is **orphan-on-format-change** — the safest option, because a stale broad grant fails closed rather than being silently re-homed into the wrong project (a one-time migration that *guessed* the originating project would risk re-granting a toy-project approval inside production). `LILARA_PROJECT_ID` overrides the derived scope (e.g. to deliberately share trust across the sub-projects of a monorepo).

  **Rug-pull pin (`runtime/mcp-pin.js`) intentionally NOT project-scoped:** it pins an arg-shape baseline keyed by `<server>/<tool>` and is a fail-open *drift detector*, not a trust *grant*. The L6 leak class — an approval in project A permitting an action in project B — does not apply to it: an MCP server's identity (and any rug-pull of its arg shape) is a global property of the server, so a shared cross-project baseline yields strictly *more* detection coverage, never a false permit. Server-name keying makes project scope orthogonal; scoping it would only *lose* the rug-pull signal the first time a compromised server is hit from a new project.

  New gate `scripts/check-project-scope.sh` (wired into `lilara-cli.sh check`) proves four properties against the real policy-store API: cross-project isolation (approval in A does **not** apply in B), no in-project regression (approval in A still applies in A), unknown/unstable scope → never matches (no false-permit fallback), and legacy unscoped entries are orphaned. `runtime/decision-engine.js` consumes auto-allow-once under the project-scoped key (the unscoped `policyKey` stays the receipt label). Script count baselines 94→95. Eval FP 0.0% / FN 0.0%; fixtures 390/390 (395/395 after ADR-020); replay corpus clean; runtime-core green.

### Fixed

- **fix(notify-transport.test.js): Windows timing flake on 50ms fire-and-forget assertion** — `tests/runtime/notify-transport.test.js:349` asserted `dt_ms < 50` for the e2e fire-and-forget latency measurement. On Windows CI the FS round-trips (`fs.appendFileSync` for journal + session state) push the hot path to ~40ms each, and the test flapped at 112ms in 1-in-4 runs post-#96 wiring. The file already uses 200ms for a related assertion at line ~282. Fix: branch the threshold on `process.platform === "win32"`: 200ms on Windows, 50ms elsewhere. The assertion is retained (not removed) — it guards that `decide()` is fire-and-forget, not absent.

- **fix(check-decide-on-every-call): gate fails when `LILARA_DECISION_JOURNAL=0` is in parent env** — `scripts/check-decide-on-every-call.sh` uses journal-write detection as its sole observable signal (delta in journal-file line count). When `LILARA_DECISION_JOURNAL=0` is inherited (e.g. from PR #91 triage-isolation hygiene), `decision-journal.js:append()` returns `false` immediately — no lines written, every command falsely reports "NO journal entry — decide() was not called" even though `decide()` ran correctly. Fix: force `process.env.LILARA_DECISION_JOURNAL = "1"` in the embedded node script alongside the existing env setup (mirrors `tests/runtime/post-adapter-harness-payloads.test.js:83`); also `delete process.env.ARG_DECISION_JOURNAL` (deprecated alias that also suppresses writes) and `delete process.env.LILARA_READONLY_CONTRACT` (=1 suppresses writes). Gate now exits 0 both with and without `LILARA_DECISION_JOURNAL=0` in the parent env.
...
- **fix(mcp-hardening): Unicode dual-path, arg-shape coverage, MultiEdit F26, graduated gate, rug-pull seam (post-PR70 hardening pass)** — Post-merge adversarial review of F25/F26 floors found four defects + two documented gaps, all fixed fail-safe:
  (1) **Fix A — Unicode dual-path in F25 + F26**: `classifyCommand` in the MCP path was raw ASCII only — Cyrillic `рm -rf /`, full-width `ｒｍ`, and homoglyph substitutions bypassed F25/F26 entirely. Added `classifyCommandDual(str)` to `runtime/decision-key.js` (ASCII fast-path: returns immediately on a raw match; second pass only when the raw result is generic AND `normalizeCommand` changes the string). Applied to all three `classifyCommand` call sites in `_evalMcpArgFloor` (F25) and `_evalMcpRegistrationFloor` (F26). Mirrors `risk-score.js` ADR-008 dual-path semantics; ASCII regexes unchanged.
  (2) **Fix B — F25 arg-shape coverage**: `rawArgs = tool_input ?? args ?? params` missed the `arguments` key (common MCP envelope shape) and `??` let a present-but-empty `tool_input:{}` mask a dangerous `args` payload. Changed to union all present containers (`tool_input`, `args`, `params`, `arguments`, `input`).
  (3) **Fix C — F26 content coverage for MultiEdit**: `_evalMcpRegistrationFloor` read only `input.content/new_string/file_text`. A MultiEdit `edits[].new_string` to `.mcp.json` was silently skipped (`content===""` → `fire:false`). Added `_collectMcpWriteContent()` helper that gathers Write, Edit, and MultiEdit `edits[].new_string` (top-level and `tool_input`-nested). NotebookEdit excluded: `notebook_path` is never a `mcpConfig`-classified path → arm would be dead code. Path extraction in the gate updated symmetrically.
  (4) **Fix D (P1) — Graduated gate for dual-use classes**: `_GATED_CMD_CLASSES` (F25 arg path) was the unambiguous-only set. Added `GATED_REVIEW_CLASSES = {destructive-db, auto-download, global-pkg-install}` — these are legitimate INPUT DATA for DB/code-assist/package MCP servers, so they reach a **require-review** gate (never a non-demotable hard block, never a blind allow). F26 registration intentionally excluded: `npx -y @scope/mcp-server` is the standard MCP server launch pattern.
  (5) **Fix E (P2) — Decouple F25 opt-out from F4 opt-out**: `policy:allow` previously silently skipped F25 for a trusted server — a rug-pulled "trusted" server could pass `rm -rf /` as an arg unblocked. Now DECOUPLED: `policy:allow` keeps suppressing F4 secret scan unchanged (credential args are legitimate for trusted servers); for F25, a **HARD_BLOCK class** arg on an allow-listed server degrades to `require-review` (auditable gate), not silent allow. `GATED_REVIEW` and generic args on trusted servers are unaffected — their legitimate data path is preserved. **Known residual (explicit):** a rug-pulled trusted DB server with `DROP DATABASE prod` in args resolves to `allow` (GATED_REVIEW + policy:allow → allow), because that is the DB connector's legitimate data. The rug-pull pin (`mcp-pin.js`) is the only detection signal for shape-drift in that path. Documented in code comments.
  - `runtime/decision-key.js` — `classifyCommandDual`, `HARD_BLOCK_CLASSES`, `GATED_REVIEW_CLASSES` added + exported.
  - `runtime/decision-engine.js` — `_collectMcpWriteContent` added; `_evalMcpArgFloor` rewritten (B+D+E); `_evalMcpRegistrationFloor` updated (A+C); F25 call-site handles new `review` return.
  - New fixtures: `tests/fixtures/mcp-security/06-f25-unicode-arg-danger.input`, `07-f26-multiedit-mcp-config.input`, `08-f25-dual-use-drop-table.input`. 389 fixtures total (was 386).
  - New eval-corpus entries: `dangerous-33` (Unicode Cyrillic arg → block FN control), `safe-54` (benign DB MCP SELECT → allow FP control). 114 entries total (was 112). FP 0.0% / FN 0.0%.
  - New tests: T8–T14 in `tests/runtime/mcp-floor-adversarial.test.js` (Unicode block; `arguments`-shape block; empty `tool_input` masking; MultiEdit F26 block; dual-use require-review; allow-listed HARD_BLOCK→require-review; allow-listed benign→allow).

- **fix(result-injection): correct stale comment + add harness-agnostic regression test (Item #1)** — `runtime/post-adapter-factory.js` block 2d (MCP result-injection) fires on `sourceLabel(toolName)==="mcp"` with no per-harness branching — the scan is harness-agnostic. All 6 harnesses have wired adapters (`check-post-adapter-parity.sh` enforces it). The previous comment ("Codex/ClawCode/Antegravity lack PostToolUse hooks") was factually wrong. Corrected to state: scan runs for all 6 wired adapters; effective coverage depends on MCP-output dispatch per harness (full on Claude Code; partial on OpenCode/OpenClaw/Codex; adapters wired but unverified live on ClawCode/Antegravity). F23 mirror comment (provenance, `:138-140`) updated consistently. `references/owasp-agentic-coverage.md` ASI01/ASI04 wording reconciled with ASI05 (all 6 adapters; variation is MCP-output dispatch reliability, not PostToolUse absence). New `tests/runtime/post-adapter-mcp-injection.test.js` (5 assertions: claude/codex/antegravity each journal `mcp-result-injection` for MCP tool output; benign output → no entry; non-MCP tool → gate holds). Wired into `scripts/check-runtime-core.sh`.
...
- **fix(mcp-pin): re-pin on drift with changeCount (Fix 4, observe-only)** — `checkArgShapeDrift` previously returned `{ drift: true }` without updating the stored hash, so every subsequent call (3rd, 4th, 5th, …) also reported `drift:true` permanently. Fixed: the drift branch now re-pins (`pins[key] = { hash, seenAt, lastChangedAt, changeCount }` + `_writePins`) so the next identical-shape call correctly returns `drift:false`. Added `changeCount` (incremented per distinct shape change, initialized to `0` on first sight) to support dashboards and future throttling — observe-only, no blocking behavior change. Three new assertions added to `tests/runtime/mcp-pin.test.js` covering re-pin-same-shape-no-drift and second-change-increments-changeCount-to-2. `mcp-pin.test.js` now reports 9 passed.
