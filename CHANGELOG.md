...
- **feat(tamper-floor): F30 `tamper-floor` — inviolable block over installed guard footprint** — `[LATTICE-BASELINE-REBASELINE]` — NEW runtime tamper floor (F30, ADR-050, rung 18.75, tier inviolable). Path-set anchored on state-paths.js resolved dirs (`stateDir()`, `hookStateDir()`, `instinctDir()`) — prefix-match with `path.sep` boundary, no hardcoded file list. `demotableBy: []`; legitimate updates route through installer/upgrade path as operator-on-host, OUTSIDE agent session (SCOPE §21 host-trust boundary). Pure predicate (no I/O); `evaluateFloor(input)` returns `{ fired: true, action: "block", reason }` when target resolves under a protected dir, else `{ fired: false }`. Engine wiring via `floor-tamper.js` module (F19/output-exfil.js encapsulation pattern); F30 fires in the rung-18.75 block band after F28 and before F16. `INVIOLABLE_FLOOR_IDS` auto-derived (21→22 inviolable floors). NEW `runtime/floor-tamper.js` (~75 LOC); NEW `tests/runtime/floor-tamper.test.js` (7 cases: block/allow/override/subdir/unrelated prefix). Lattice baseline rebaseline required (F30 added to inviolable set). §19 #6 precondition satisfied on master.
