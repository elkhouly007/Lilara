#!/usr/bin/env bash
# check-antegravity-adapter.sh — Verify the antegravity adapter hook.
#
# antegravity's hook API is not publicly documented so this adapter uses a
...
#
# Usage: bash scripts/check-antegravity-adapter.sh
...
if ! command -v node >/dev/null 2>&1; then
  printf '[check-antegravity-adapter] node not found — skipping\n' >&2
  exit 0
fi
...
printf '[check-antegravity-adapter]\n'

adapter="$root/antegravity/hooks/adapter.js"
...
# ── 1: file exists ────────────────────────────────────────────────────────────
[ -f "$adapter" ] || fail "antegravity/hooks/adapter.js missing"
