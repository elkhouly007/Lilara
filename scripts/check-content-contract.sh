...
#      (ADR-051; Red Lines A & B; SCOPE §4-§8) — not from the template's own
#      wording. The template is validated AGAINST the spec, avoiding circular
#      self-validation.
...
const required = [
  ...
  // absolute tier (§7) — incl. Red Lines A & B
  ...
  ["absolute: Red Line B (fabricated real person)", /fabricated or manipulated depiction of a real\s*specific person/i],
  ["absolute: Red Line B methods", /face-swap, deepfake, montage/i],
  ["absolute: Red Line B blanket (benign)", /benign-looking\s*edits/i],
  ["absolute: Red Line B not consent/fame", /regardless of any consent claim and whether the person is public or private/i],
  ["absolute: Red Line B generic exclusion", /generic,\s*non-identifiable person is not covered/i],
...
assert(headerVer === "1.1.0", "contract header version is 1.1.0 (got " + headerVer + ")");
