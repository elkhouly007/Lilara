...
# ── file-write-floor fixtures (F24 credential-persistence-write + universal scoring) ─────
# Exercises F24 hard floor, scoring arms, contract opt-out, and F16/F24 boundary.
printf '\n[file-write-floor]\n'
...
if bash scripts/check-file-write-floor.sh > "$_fwf_tmp_out" 2>&1; then
  while IFS= read -r line; do
    case "$line" in
      "  ok      "*)
        ok "file-write-floor:${line#"  ok      "}"
        ;;
      "  FAIL    "*)
        body="${line#"  FAIL    "}"
        name="${body%% — *}"
        rest="${body#* — }"
        fail "file-write-floor:$name" "$rest"
        ;;
    esac
  done < "$_fwf_tmp_out"
else
