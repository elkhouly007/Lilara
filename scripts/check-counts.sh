...
EXPECTED_FIXTURES=406      # fixture pairs; +11 for file-write-floor F24 fixtures; +3 for mcp-security (F25/F4-opt-out/benign); +2 for F26 raw-value fallback; +3 for hardening: 06-unicode-arg-danger, 07-multiedit-mcp-config, 08-dual-use-drop-table; +1 for 09-f25-ifs-arg-danger (${IFS} whitespace-evasion fold); +5 for ADR-020 narrow (10/11/12/13/14); +1 for ADR-018 fixture A (15-f25-trusted-dualuse-nodrift-allow); +4 for Track B confusable bypass (16-cyrillic-base64/17-zwj-base64/18-cyrillic-proc-sub block + 19-variable-template allow); +2 for ADR-036 inviolable tier (F27-secret-egress-external.input + inviolable-tier/f20-floor-codes-policy-edit.input); +4 for P2.6 hermes fixtures (hermes-{safe-ls,dangerous-rm-rf,enforce-rm-rf,borderline-sudo}.input)
EXPECTED_SCRIPTS=108       # sh + js files in scripts/; +check-file-write-floor.sh +dashboard-server.js +check-dashboard.sh +check-mcp-security.sh +check-project-scope.sh (L6) +journal-tail.js (Item 5, pre-testing-readiness sprint) +check-consent-gate.sh (0.2.0 consent gate) +check-inviolable-tier.sh (ADR-036) +check-install-smoke.sh (fix distribution gap) +check-taint-egress.sh (ADR-037 F28) +check-delete-coord.sh (ADR-038 F29) +check-scope-tags.sh (SCOPE.md decision-tag integrity, R2 scope review) +check-content-contract.sh (enforcement point (b) conformance, ADR-051) +check-eval-budgets.sh (Phase 1 P1.3 — advisory slice-quality gate, ADVISORY only) +check-inviolable-tier-unreachability.sh (P3.1 — §19 #6 laundering regression gate, axis (a) structural unreachability + axis (b) monotonic rebaseline-marker check) +check-replay-posture-matrix.sh (P3.3 — §19 #14 posture-matrix replay: 8 posture combinations × N corpora, all must be zero-drift) +branch-hygiene.sh (2026-06-21 housekeeping — enumerate merged remote + stale local branches; --apply to delete) +check-f30-tamper-floor.sh (ADR-050 F30 runtime tamper-floor structural gate)
