...
EXPECTED_FIXTURES=395      # fixture pairs; +11 for file-write-floor F24 fixtures; +3 for mcp-security (F25/F4-opt-out/benign); +2 for F26 raw-value fallback; +3 for hardening: 06-unicode-arg-danger, 07-multiedit-mcp-config, 08-dual-use-drop-table; +1 for 09-f25-ifs-arg-danger (${IFS} whitespace-evasion fold); +5 for ADR-020 narrow (10/11/12/13/14)
