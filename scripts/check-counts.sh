...
EXPECTED_SCRIPTS=103       # sh + js files in scripts/; +check-file-write-floor.sh +dashboard-server.js +check-dashboard.sh +check-mcp-security.sh +check-project-scope.sh (L6) +journal-tail.js (Item 5, pre-testing-readiness sprint) +check-consent-gate.sh (0.2.0 consent gate) +check-inviolable-tier.sh (ADR-036) +check-install-smoke.sh (fix distribution gap) +check-taint-egress.sh (ADR-037 F28) +check-delete-coord.sh (ADR-038 F29) +check-scope-tags.sh (SCOPE.md decision-tag integrity, R2 scope review) +check-content-contract.sh (enforcement point (b) conformance, ADR-051)
