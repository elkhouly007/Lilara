...
EXPECTED_FIXTURES=402      # fixture pairs; +11 for file-write-floor F24 fixtures; +3 for mcp-security (F25/F4-opt-out/benign); +2 for F26 raw-value fallback; +3 for hardening: 06-unicode-arg-danger, 07-multiedit-mcp-config, 08-dual-use-drop-table; +1 for 09-f25-ifs-arg-danger (${IFS} whitespace-evasion fold); +5 for ADR-020 narrow (10/11/12/13/14); +1 for ADR-018 fixture A (15-f25-trusted-dualuse-nodrift-allow); +4 for Track B confusable bypass (16-cyrillic-base64/17-zwj-base64/18-cyrillic-proc-sub block + 19-variable-template allow); +2 for ADR-036 inviolable tier (F27-secret-egress-external.input + inviolable-tier/f20-floor-codes-policy-edit.input)
