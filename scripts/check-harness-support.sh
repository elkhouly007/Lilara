...
# No planned harnesses remain — Codex, antegravity, and Claw Code are all verified.
for harness in; do
  grep -Fq "$harness" "$readme" || fail "README Harness Support Matrix missing planned harness: $harness"
done
pass 'No remaining planned harnesses'
...
# Codex is now VERIFIED — it must NOT carry the planned-stub marker and
# MUST carry a WIRING_PLAN.md authored against verified source.
[ -f "$root/codex/README.md" ] || fail "codex/README.md missing"
grep -qE 'NOT YET SUPPORTED|EXPERIMENTAL' "$root/codex/README.md" \
  && fail "codex/README.md still carries planned-stub marker after verification (2026-05-24)"
grep -qE 'VERIFIED' "$root/codex/README.md" \
  || fail "codex/README.md missing VERIFIED marker"
[ -f "$root/codex/WIRING_PLAN.md" ] || fail "codex/WIRING_PLAN.md missing — required for verified harness"
[ -f "$root/codex/COMPATIBILITY_NOTES.md" ] || fail "codex/COMPATIBILITY_NOTES.md missing"
pass 'codex verified directory layout present (README + WIRING_PLAN + COMPATIBILITY_NOTES)'

# Antegravity is now VERIFIED — it must NOT carry the planned-stub marker and
# MUST carry a WIRING_PLAN.md authored against verified source.
[ -f "$root/antegravity/README.md" ] || fail "antegravity/README.md missing"
grep -qE 'NOT YET SUPPORTED|EXPERIMENTAL' "$root/antegravity/README.md" \
  && fail "antegravity/README.md still carries planned-stub marker after verification (2026-05-24)"
grep -qE 'VERIFIED' "$root/antegravity/README.md" \
  || fail "antegravity/README.md missing VERIFIED marker"
[ -f "$root/antegravity/WIRING_PLAN.md" ] || fail "antegravity/WIRING_PLAN.md missing — required for verified harness"
...
grep -Fq 'codex' "$apply_status" || fail 'per-tool-apply-status.md missing codex entry'
grep -Fq 'clawcode' "$apply_status" || fail 'per-tool-apply-status.md missing clawcode entry'
grep -Fq 'antegravity' "$apply_status" || fail 'per-tool-apply-status.md missing antegravity entry'
pass 'per-tool-apply-status.md has Planned Harnesses section with all entries'
