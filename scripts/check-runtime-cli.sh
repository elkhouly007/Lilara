...
# and the fixture repos (parent of repo/, protected-repo/, shape-repo/,
# tests-shape-repo/) in SEPARATE tmp dirs. Previously tmp_home was used
# as BOTH HOME and LILARA_STATE_DIR and as the parent of the fixture
# repos, which put repo/<anything> inside F30's protected footprint —
# F30 then correctly blocked on targetPath=$repo/... and masked the
# runtime decisions this script is designed to exercise. See PR-F step
# 1 (anti-mask refactor) — same pattern already applied to
# tests/runtime/{degraded-mode,journal-chain,taint-window-injection,
# consent-f6-prompt-wiring}.test.js and to scripts/check-config-integration.sh.
state_dir="$(mktemp -d)"
repo_root="$(mktemp -d)"
dismiss_state="$(mktemp -d)"
aao_state="$(mktemp -d)"
cleanup() { rm -rf "$state_dir" "$repo_root" "$dismiss_state" "$aao_state" "${dismiss_home:-}" "${aao_home:-}"; }
trap cleanup EXIT

HOME="$repo_root" LILARA_STATE_DIR="$state_dir" node - <<'NODE' "$root" || exit 1
...
NODE
policy_key="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" node - <<'NODE' "$root"
const path = require('path');
const runtime = require(path.join(process.argv[2], 'runtime'));
const suggestions = runtime.listSuggestions();
...
state_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime state)"
printf '%s\n' "$state_output" | grep -q 'pending-suggestions: 1' || fail 'runtime state shows pending suggestion count'
printf '%s\n' "$state_output" | grep -q "promote: lilara-cli.sh runtime promote '$policy_key'" || fail 'runtime state shows explicit promote command'
pass 'runtime state shows suggestion summary'

HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime accept "$policy_key" >/dev/null
state_after_accept="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime state)"
...
HOME="$repo_root" LILARA_STATE_DIR="$state_dir" node - <<'NODE' "$root" >/dev/null
const path = require('path');
const runtime = require(path.join(process.argv[2], 'runtime'));
...
runtime.recordApproval(input);
NODE
promote_key="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" node - <<'NODE' "$root"
const path = require('path');
const runtime = require(path.join(process.argv[2], 'runtime'));
const suggestions = runtime.listSuggestions();
...
HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime promote "$promote_key" >/dev/null
promoted_state="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime state)"
...
record_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime record-approval --tool Bash --command 'sudo systemctl restart app' --target ops/service)"
printf '%s\n' "$record_output" | grep -q 'Recorded approval:' || fail 'runtime record-approval prints result'
pass 'runtime record-approval works'

repo="$repo_root/repo"
mkdir -p "$repo/ops"
git -C "$repo_root" init repo >/dev/null 2>&1
git -C "$repo" checkout -b release >/dev/null 2>&1
cat > "$repo/lilara.config.json" <<'EOF'
{
  "runtime": {
...
protected_repo="$repo_root/protected-repo"
mkdir -p "$protected_repo/ops"
git -C "$repo_root" init protected-repo >/dev/null 2>&1
git -C "$protected_repo" checkout -b release >/dev/null 2>&1
cat > "$protected_repo/lilara.config.json" <<'EOF'
{
  "runtime": {
    "trust_posture": "balanced",
    "protected_branches": ["release"]
...
shape_repo="$repo_root/shape-repo"
mkdir -p "$shape_repo/src"
git -C "$repo_root" init shape-repo >/dev/null 2>&1
...
tests_shape_repo="$repo_root/tests-shape-repo"
mkdir -p "$tests_shape_repo"
git -C "$repo_root" init tests-shape-repo >/dev/null 2>&1
...
HOME="$repo_root" LILARA_STATE_DIR="$state_dir" node - <<NODE "$root" "$repo" >/dev/null
const path = require('path');
const runtime = require(path.join(process.argv[2], 'runtime'));
...
explain_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'sudo systemctl restart app' --target "$repo/ops" )"
...
printf '%s\n' "$explain_output" | grep -q 'workflow-target: lilara-cli.review' || fail 'runtime explain prints workflow target'
printf '%s\n' "$explain_output" | grep -q 'workflow-command: lilara-cli.sh review' || fail 'runtime explain prints workflow command'
...
modify_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'cat prod/config' --target "$repo/prod/service" --branch feature/payments )"
...
HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime record-approval --tool Bash --command "$destructive_cmd" --target /tmp/build >/dev/null
HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime record-approval --tool Bash --command "$destructive_cmd" --target /tmp/build >/dev/null
adaptive_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command "$destructive_cmd" --target /tmp/build)"
...
stack_tests_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command "$destructive_cmd" --target "$tests_shape_repo/build" --projectRoot "$tests_shape_repo")"
...
route_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'npm test' --target web/app.ts --branch feature/ci-test)"
...
source_route_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'update module' --target src/runtime/app.ts --branch feature/ci-test)"
...
source_shape_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'update module' --target "$shape_repo/src/app.ts" --projectRoot "$shape_repo")"
...
source_edit_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Edit --command 'edit module' --target src/runtime/app.ts --branch feature/ci-test)"
...
strict_source_route_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'update module' --target src/runtime/app.ts --projectRoot "$repo")"
...
printf '%s\n' "$strict_source_route_output" | grep -q 'workflow-target: lilara-cli.review' || fail 'runtime explain prints strict source-file review target'

protected_source_route_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'update module' --target src/runtime/app.ts --projectRoot "$protected_repo")"
...
strict_source_edit_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Edit --command 'edit module' --target src/runtime/app.ts --projectRoot "$repo")"
...
printf '%s\n' "$strict_source_edit_output" | grep -q 'workflow-target: lilara-cli.review' || fail 'runtime explain prints strict direct source edit review target'

protected_source_edit_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Edit --command 'edit module' --target src/runtime/app.ts --projectRoot "$protected_repo")"
...
printf '%s\n' "$protected_source_edit_output" | grep -q 'workflow-reason: This target looks like source code on protected branch release, so direct edits should route through review first.' || fail 'runtime explain prints protected source edit review reason'
 ...
 docs_route_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'update docs' --target docs/runtime-notes.md --branch feature/ci-test)"
printf '%s\n' "$docs_route_output" | grep -q 'workflow-lane: direct' || fail 'runtime explain keeps docs updates on direct lane by default'

setup_route_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'setup profile full' --target lilara.config.json --branch feature/ci-test)"
...
setup_shape_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'setup project' --target "$shape_repo" --projectRoot "$shape_repo")"
...
printf '%s\n' "$setup_shape_output" | grep -q 'workflow-target: lilara-cli.generate-config' || fail 'runtime explain routes missing-config setup into generate-config target'
...
wiring_edit_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Edit --command 'edit settings' --target .claude/settings.json --branch feature/hooks)"
printf '%s\n' "$wiring_edit_output" | grep -q 'workflow-lane: wiring' || fail 'runtime explain routes direct settings edits into wiring lane'
printf '%s\n' "$wiring_edit_output" | grep -q 'workflow-target: lilara-cli.wire' || fail 'runtime explain prints wiring target for direct settings edits'
...
strict_wiring_edit_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Edit --command 'edit settings' --target .claude/settings.json --projectRoot "$repo")"
printf '%s\n' "$strict_wiring_edit_output" | grep -q 'workflow-lane: review' || fail 'runtime explain routes strict wiring edits into review lane'
printf '%s\n' "$strict_wiring_edit_output" | grep -q 'workflow-target: lilara-cli.review' || fail 'runtime explain prints strict wiring review target'

protected_wiring_edit_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Edit --command 'edit settings' --target .claude/settings.json --projectRoot "$protected_repo")"
...
payload_route_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'redact payload.json' --target payload.json)"
...
class_b_route_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'open customer export' --target exports/customer.csv --payloadClass B --branch feature/ci-test)"
...
class_c_demote_token="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" operator-token mint test-class-c-route --scope class-c-review-demote 2>/dev/null | awk -F': ' '/^Token:/ {print $2}')"
class_c_route_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" LILARA_F4_DEMOTE_TOKEN="$class_c_demote_token" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'inspect incident bundle' --target bundle.zip --payloadClass C --branch feature/incidents)"
...
printf '%s\n' "$class_c_route_output" | grep -q 'workflow-target: lilara-cli.review' || fail 'runtime explain prints class C review target'

# ADR-002 Option B negative: without operator-token, class C stays blocked.
class_c_blocked_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'inspect incident bundle' --target bundle.zip --payloadClass C --branch feature/incidents)"
...
classify_route_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'classify payload.json' --target payload.json --branch feature/ci-test)"
printf '%s\n' "$classify_route_output" | grep -q 'workflow-target: lilara-cli.classify' || fail 'runtime explain prints payload classify target'

payload_review_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'review payload.json' --target payload.json --branch feature/ci-test)"
...
review_route_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'security review auth diff' --target changes.patch)"
...
escalate_route_output="$(HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'git push --force origin main' --target src/app.ts --branch feature/ci-test)"
...
# Dismissed-defaults block: separate LILARA_STATE_DIR from the dismiss cwd.
# dismiss_home is the HOME (cwd for the relative-path targetPath used here);
# dismiss_state is the guard state write site, in its own dir so F30's
# protected footprint does not include $dismiss_home.
dismiss_home="$(mktemp -d)"
cp -r "$repo_root/." "$dismiss_home/"
HOME="$dismiss_home" LILARA_STATE_DIR="$dismiss_state" bash "$root/scripts/lilara-cli.sh" runtime record-approval --tool Bash --command 'npx -y tsx scripts/cache.ts' --target production/cache.ts >/dev/null
HOME="$dismiss_home" LILARA_STATE_DIR="$dismiss_state" bash "$root/scripts/lilara-cli.sh" runtime record-approval --tool Bash --command 'npx -y tsx scripts/cache.ts' --target production/cache.ts >/dev/null
HOME="$dismiss_home" LILARA_STATE_DIR="$dismiss_state" bash "$root/scripts/lilara-cli.sh" runtime record-approval --tool Bash --command 'npx -y tsx scripts/cache.ts' --target production/cache.ts >/dev/null
dismiss_key="$(HOME="$dismiss_home" LILARA_STATE_DIR="$dismiss_state" node - <<'NODE' "$root"
const path = require('path');
const runtime = require(path.join(process.argv[2], 'runtime'));
...
HOME="$dismiss_home" LILARA_STATE_DIR="$dismiss_state" bash "$root/scripts/lilara-cli.sh" runtime dismiss "$dismiss_key" >/dev/null
dismissed_state="$(HOME="$dismiss_home" LILARA_STATE_DIR="$dismiss_state" bash "$root/scripts/lilara-cli.sh" runtime state)"
...
dismissed_explain="$(HOME="$dismiss_home" LILARA_STATE_DIR="$dismiss_state" bash "$root/scripts/lilara-cli.sh" runtime explain --tool Bash --command 'npx -y tsx scripts/cache.ts' --target production/cache.ts)"
...
rm -rf "$dismiss_home"
HOME="$repo_root" LILARA_STATE_DIR="$state_dir" bash "$root/scripts/lilara-cli.sh" runtime dismiss 'missing-key' >/dev/null 2>&1 && fail 'runtime dismiss should fail on missing key' || true
pass 'runtime dismiss rejects unknown key'

# B.1 auto-allow-once CLI verb
# B.1 auto-allow-once CLI verb: separate LILARA_STATE_DIR from the aao cwd.
# aao_home is the HOME (cwd for the relative-path targetPath); aao_state
# is the guard state write site, in its own dir so F30's protected
# footprint does not include $aao_home.
aao_home="$(mktemp -d)"
HOME="$aao_home" LILARA_STATE_DIR="$aao_state" bash "$root/scripts/lilara-cli.sh" runtime record-approval --tool Bash --command 'npx -y tsx scripts/migrate.ts' --target scripts/ >/dev/null
HOME="$aao_home" LILARA_STATE_DIR="$aao_state" bash "$root/scripts/lilara-cli.sh" runtime record-approval --tool Bash --command 'npx -y tsx scripts/migrate.ts' --target scripts/ >/dev/null
HOME="$aao_home" LILARA_STATE_DIR="$aao_state" bash "$root/scripts/lilara-cli.sh" runtime record-approval --tool Bash --command 'npx -y tsx scripts/migrate.ts' --target scripts/ >/dev/null
aao_key="$(HOME="$aao_home" LILARA_STATE_DIR="$aao_state" node - <<'NODE' "$root"
const path = require('path');
const runtime = require(path.join(process.argv[2], 'runtime'));
const suggestions = runtime.listSuggestions().filter((item) => item.key.includes('auto-download'));
process.stdout.write(suggestions[0].key);
...
aao_output="$(HOME="$aao_home" LILARA_STATE_DIR="$aao_state" bash "$root/scripts/lilara-cli.sh" runtime auto-allow-once "$aao_key")"
printf '%s\n' "$aao_output" | grep -q "Granted auto-allow-once for:" || fail 'runtime auto-allow-once prints grant confirmation'
HOME="$aao_home" LILARA_STATE_DIR="$aao_state" bash "$root/scripts/lilara-cli.sh" runtime auto-allow-once 'bash|generic|default-target|A' >/dev/null 2>&1 && fail 'runtime auto-allow-once should fail for non-eligible key' || true
rm -rf "$aao_home"
