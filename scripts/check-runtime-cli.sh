...
NODE
policy_key="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" node - <<'NODE' "$root"
const path = require('path');
const runtime = require(path.join(process.argv[2], 'runtime'));
const suggestions = runtime.listSuggestions();
...
state_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime state)"
printf '%s\n' "$state_output" | grep -q 'pending-suggestions: 1' || fail 'runtime state shows pending suggestion count'
printf '%s\n' "$state_output" | grep -q "promote: horus-cli.sh runtime promote '$policy_key'" || fail 'runtime state shows explicit promote command'
pass 'runtime state shows suggestion summary'

HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime accept "$policy_key" >/dev/null
state_after_accept="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime state)"
...
HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" node - <<'NODE' "$root" >/dev/null
const path = require('path');
const runtime = require(path.join(process.argv[2], 'runtime'));
...
runtime.recordApproval(input);
NODE
promote_key="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" node - <<'NODE' "$root"
const path = require('path');
const runtime = require(path.join(process.argv[2], 'runtime'));
const suggestions = runtime.listSuggestions();
...
HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime promote "$promote_key" >/dev/null
promoted_state="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime state)"
...
record_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime record-approval --tool Bash --command 'sudo systemctl restart app' --target ops/service)"
...
cat > "$repo/horus.config.json" <<'EOF'
{
  "runtime": {
...
cat > "$protected_repo/horus.config.json" <<'EOF'
{
  "runtime": {
    "trust_posture": "balanced",
    "protected_branches": ["release"]
...
HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" node - <<NODE "$root" "$repo" >/dev/null
const path = require('path');
const runtime = require(path.join(process.argv[2], 'runtime'));
...
explain_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Bash --command 'sudo systemctl restart app' --target "$repo/ops" )"
...
printf '%s\n' "$explain_output" | grep -q 'workflow-target: horus-cli.review' || fail 'runtime explain prints workflow target'
printf '%s\n' "$explain_output" | grep -q 'workflow-command: horus-cli.sh review' || fail 'runtime explain prints workflow command'
...
modify_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Bash --command 'cat prod/config' --target "$repo/prod/service" --branch feature/payments )"
...
HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime record-approval --tool Bash --command "$destructive_cmd" --target /tmp/build >/dev/null
HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime record-approval --tool Bash --command "$destructive_cmd" --target /tmp/build >/dev/null
adaptive_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Bash --command "$destructive_cmd" --target /tmp/build)"
...
stack_tests_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Bash --command "$destructive_cmd" --target "$tests_shape_repo/build" --projectRoot "$tests_shape_repo")"
...
route_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Bash --command 'npm test' --target web/app.ts)"
printf '%s\n' "$route_output" | grep -q 'workflow-lane: checks' || fail 'runtime explain routes low-risk verification into checks lane'
printf '%s\n' "$route_output" | grep -q 'workflow-target: horus-cli.check' || fail 'runtime explain prints checks workflow target'
printf '%s\n' "$route_output" | grep -q 'workflow-command: horus-cli.sh check' || fail 'runtime explain prints checks workflow command'

source_route_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Bash --command 'update module' --target src/runtime/app.ts)"
printf '%s\n' "$source_route_output" | grep -q 'workflow-lane: checks' || fail 'runtime explain routes source-file work into checks lane'
printf '%s\n' "$source_route_output" | grep -q 'workflow-target: horus-cli.check' || fail 'runtime explain prints source-file checks target'

source_shape_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Bash --command 'update module' --target "$shape_repo/src/app.ts" --projectRoot "$shape_repo")"
...
printf '%s\n' "$source_shape_output" | grep -q 'workflow-command: horus-cli.sh check && npm test' || fail 'runtime explain prints stack-aware source route command'

source_edit_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Edit --command 'edit module' --target src/runtime/app.ts)"
...
strict_source_route_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Bash --command 'update module' --target src/runtime/app.ts --projectRoot "$repo")"
...
printf '%s\n' "$strict_source_route_output" | grep -q 'workflow-target: horus-cli.review' || fail 'runtime explain prints strict source-file review target'

protected_source_route_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Bash --command 'update module' --target src/runtime/app.ts --projectRoot "$protected_repo")"
...
strict_source_edit_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Edit --command 'edit module' --target src/runtime/app.ts --projectRoot "$repo")"
...
printf '%s\n' "$strict_source_edit_output" | grep -q 'workflow-target: horus-cli.review' || fail 'runtime explain prints strict direct source edit review target'

protected_source_edit_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Edit --command 'edit module' --target src/runtime/app.ts --projectRoot "$protected_repo")"
...
printf '%s\n' "$protected_source_edit_output" | grep -q 'workflow-reason: This target looks like source code on protected branch release, so direct edits should route through review first.' || fail 'runtime explain prints protected source edit review reason'
 ...
 docs_route_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Bash --command 'update docs' --target docs/runtime-notes.md)"
printf '%s\n' "$docs_route_output" | grep -q 'workflow-lane: direct' || fail 'runtime explain keeps docs updates on direct lane by default'

setup_route_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Bash --command 'setup profile full' --target horus.config.json)"
printf '%s\n' "$setup_route_output" | grep -q 'workflow-lane: setup' || fail 'runtime explain routes setup work into setup lane'
printf '%s\n' "$setup_route_output" | grep -q 'workflow-target: horus-cli.setup' || fail 'runtime explain prints setup workflow target'
printf '%s\n' "$setup_route_output" | grep -q 'workflow-command: horus-cli.sh setup' || fail 'runtime explain prints setup workflow command'

setup_shape_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Bash --command 'setup project' --target "$shape_repo" --projectRoot "$shape_repo")"
...
printf '%s\n' "$setup_shape_output" | grep -q 'workflow-target: horus-cli.generate-config' || fail 'runtime explain routes missing-config setup into generate-config target'
...
wiring_edit_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Edit --command 'edit settings' --target .claude/settings.json --branch feature/hooks)"
printf '%s\n' "$wiring_edit_output" | grep -q 'workflow-lane: wiring' || fail 'runtime explain routes direct settings edits into wiring lane'
printf '%s\n' "$wiring_edit_output" | grep -q 'workflow-target: horus-cli.wire' || fail 'runtime explain prints wiring target for direct settings edits'
...
strict_wiring_edit_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Edit --command 'edit settings' --target .claude/settings.json --projectRoot "$repo")"
printf '%s\n' "$strict_wiring_edit_output" | grep -q 'workflow-lane: review' || fail 'runtime explain routes strict wiring edits into review lane'
printf '%s\n' "$strict_wiring_edit_output" | grep -q 'workflow-target: horus-cli.review' || fail 'runtime explain prints strict wiring review target'

protected_wiring_edit_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Edit --command 'edit settings' --target .claude/settings.json --projectRoot "$protected_repo")"
...
payload_route_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Bash --command 'redact payload.json' --target payload.json)"
printf '%s\n' "$payload_route_output" | grep -q 'workflow-lane: payload' || fail 'runtime explain routes payload work into payload lane'
printf '%s\n' "$payload_route_output" | grep -q 'workflow-target: horus-cli.redact' || fail 'runtime explain prints payload redact target'
printf '%s\n' "$payload_route_output" | grep -q 'workflow-command: horus-cli.sh redact <file>' || fail 'runtime explain prints payload workflow command'

class_b_route_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Bash --command 'open customer export' --target exports/customer.csv --payloadClass B)"
...
payload_review_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Bash --command 'review payload.json' --target payload.json)"
printf '%s\n' "$payload_review_output" | grep -q 'workflow-lane: payload' || fail 'runtime explain keeps payload review in payload lane'
printf '%s\n' "$payload_review_output" | grep -q 'workflow-target: horus-cli.review' || fail 'runtime explain prints payload review target'
printf '%s\n' "$payload_review_output" | grep -q 'workflow-command: horus-cli.sh review <file>' || fail 'runtime explain prints payload review command'

review_route_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Bash --command 'security review auth diff' --target changes.patch)"
printf '%s\n' "$review_route_output" | grep -q 'workflow-lane: review' || fail 'runtime explain routes review work into review lane'
printf '%s\n' "$review_route_output" | grep -q 'workflow-target: horus-cli.review' || fail 'runtime explain prints review workflow target'
printf '%s\n' "$review_route_output" | grep -q 'workflow-command: horus-cli.sh review' || fail 'runtime explain prints review workflow command'
escalate_route_output="$(HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Bash --command 'git push --force origin main' --target src/app.ts)"
...
HOME="$dismiss_home" HORUS_STATE_DIR="$dismiss_home" bash "$root/scripts/horus-cli.sh" runtime record-approval --tool Bash --command 'npx -y tsx scripts/cache.ts' --target production/cache.ts >/dev/null
HOME="$dismiss_home" HORUS_STATE_DIR="$dismiss_home" bash "$root/scripts/horus-cli.sh" runtime record-approval --tool Bash --command 'npx -y tsx scripts/cache.ts' --target production/cache.ts >/dev/null
HOME="$dismiss_home" HORUS_STATE_DIR="$dismiss_home" bash "$root/scripts/horus-cli.sh" runtime record-approval --tool Bash --command 'npx -y tsx scripts/cache.ts' --target production/cache.ts >/dev/null
dismiss_key="$(HOME="$dismiss_home" HORUS_STATE_DIR="$dismiss_home" node - <<'NODE' "$root"
const path = require('path');
const runtime = require(path.join(process.argv[2], 'runtime'));
...
HOME="$dismiss_home" HORUS_STATE_DIR="$dismiss_home" bash "$root/scripts/horus-cli.sh" runtime dismiss "$dismiss_key" >/dev/null
dismissed_state="$(HOME="$dismiss_home" HORUS_STATE_DIR="$dismiss_home" bash "$root/scripts/horus-cli.sh" runtime state)"
...
dismissed_explain="$(HOME="$dismiss_home" HORUS_STATE_DIR="$dismiss_home" bash "$root/scripts/horus-cli.sh" runtime explain --tool Bash --command 'npx -y tsx scripts/cache.ts' --target production/cache.ts)"
...
rm -rf "$dismiss_home"
HOME="$tmp_home" HORUS_STATE_DIR="$tmp_home" bash "$root/scripts/horus-cli.sh" runtime dismiss 'missing-key' >/dev/null 2>&1 && fail 'runtime dismiss should fail on missing key' || true
pass 'runtime dismiss rejects unknown key'

# B.1 auto-allow-once CLI verb
aao_home="$(mktemp -d)"
HOME="$aao_home" HORUS_STATE_DIR="$aao_home" bash "$root/scripts/horus-cli.sh" runtime record-approval --tool Bash --command 'npx -y tsx scripts/migrate.ts' --target scripts/ >/dev/null
HOME="$aao_home" HORUS_STATE_DIR="$aao_home" bash "$root/scripts/horus-cli.sh" runtime record-approval --tool Bash --command 'npx -y tsx scripts/migrate.ts' --target scripts/ >/dev/null
HOME="$aao_home" HORUS_STATE_DIR="$aao_home" bash "$root/scripts/horus-cli.sh" runtime record-approval --tool Bash --command 'npx -y tsx scripts/migrate.ts' --target scripts/ >/dev/null
aao_key="$(HOME="$aao_home" HORUS_STATE_DIR="$aao_home" node - <<'NODE' "$root"
const path = require('path');
const runtime = require(path.join(process.argv[2], 'runtime'));
const suggestions = runtime.listSuggestions().filter((item) => item.key.includes('auto-download'));
process.stdout.write(suggestions[0].key);
...
aao_output="$(HOME="$aao_home" HORUS_STATE_DIR="$aao_home" bash "$root/scripts/horus-cli.sh" runtime auto-allow-once "$aao_key")"
printf '%s\n' "$aao_output" | grep -q "Granted auto-allow-once for:" || fail 'runtime auto-allow-once prints grant confirmation'
HOME="$aao_home" HORUS_STATE_DIR="$aao_home" bash "$root/scripts/horus-cli.sh" runtime auto-allow-once 'bash|generic|default-target|A' >/dev/null 2>&1 && fail 'runtime auto-allow-once should fail for non-eligible key' || true
rm -rf "$aao_home"
